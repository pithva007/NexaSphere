...
export const generateTrendData = (granularity: TimeGranularity, monthsBack: number = 6): TrendDataPoint[] => {
  const data: TrendDataPoint[] = [];
  const now = new Date();
  
  const points = granularity === 'monthly' ? monthsBack : granularity === 'weekly' ? monthsBack * 4 : monthsBack * 30;
  ...
  for (let i = points; i >= 0; i--) {
    ...
    const name = granularity === 'monthly' 
