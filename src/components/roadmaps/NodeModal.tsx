...
export const NodeModal: React.FC<NodeModalProps> = ({ theme }) => {
  ...
  const [status, setStatus] = useState<'Not Started' | 'In Progress' | 'Completed' | 'Stuck'>('Not Started');
  ...
  const getGlowStyle = () => {
    switch (status) {
      case 'In Progress': return 'glowing-amber-modal';
      case 'Completed': return 'glowing-emerald-modal';
      case 'Stuck': return 'glowing-ruby-modal';
      default: return '';
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Form Grid */}
        <div className="modal-form-stack" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {/* Node Title */}
          <div className="form-group">
            <label htmlFor="node-title" className="form-label text-xs font-black uppercase text-t2 block mb-2">
              Topic/Title
            </label>
            <input
              ref={titleInputRef}
          ...
          {/* Node Description */}
          <div className="form-group">
            <label htmlFor="node-desc" className="form-label text-xs font-black uppercase text-t2 block mb-2">
              Short Description
            </label>
            <textarea
              id="node-desc"
          ...
          <div className="form-group">
            ...
            <div className="status-grid-selector" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
          ...
          <div className="form-group">
            ...
            <div className="prereq-selector-scroll glassmorphic-panel" style={{ maxHeight: '120px', overflowY: 'auto', padding: '12px', borderRadius: '12px' }}>
              {nodes.filter(n => n.id !== node.id).length === 0 ? (
                <p className="text-xxs text-t3 text-center py-2">No other nodes available to establish connections.</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {nodes.filter(n => n.id !== node.id).map(otherNode => {
                    const isChecked = prerequisites.includes(otherNode.id);
                    return (
                      <label key={otherNode.id} className="prereq-checkbox-row text-xs text-t2 flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={isChecked}
          ...
          <div className="form-group">
            ...
            <div className="resources-editor-stack" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              {urlError && <p className="text-xxs text-brand-red font-bold mt-1"><AlertCircle size={10} style={{ display: 'inline', marginRight: '4px' }} />{urlError}</p>}

              {/* Render resource cards stack */}
              <div className="resource-list-stack mt-2" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {resources.map((res, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2 truncate">
                      ...
                      <a href={res.url} target="_blank" rel="noopener noreferrer" className="font-bold text-t1 hover:underline truncate">
                        {res.title}
                      </a>
                    </div>
                    <button
          ...
          <div className="form-group">
            <div style={{ display: 'flex', justifyContent: 'between', alignItems: 'center', marginBottom: '8px' }}>
              <label htmlFor="node-notes" className="form-label text-xs font-black uppercase text-t2 block">
                Personal Notes &amp; Checklist Items
              </label>
              <span className="text-xxs text-t3">Supports Markdown list notation (- [ ] for checklists)</span>
            </div>
            <textarea
              id="node-notes"
              rows={4}
